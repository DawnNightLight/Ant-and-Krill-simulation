...
for krill_number in KRILL:
    ...
    for run in range(1):
        ...
        krill = {}
        for i in range(krill_number):
            krill[i] = {"coordinates": (random.randint(0, MAP_SIZE // 4), random.randint(0, MAP_SIZE // 4)),
        ...
        videoWriter = cv2.VideoWriter("Results/krill_video.mp4", cv2.VideoWriter_fourcc('m', 'p', '4', 'v'), 24,(700, 700))
        ...
        results[run] = {"krillnum": krill_number, "time": time, "total": food_store, "min": min_value, "max": max_value}
        run += 1
        videoWriter.release()
    ...
    with open(f"Allkrill", "w", newline="") as csv_file:
        csv_file.write("krillnum, time, total, min, max")
